...
<form method="post" action="{{route('trips.destroy',[$info->detail_id])}}" style="display: none;" id="delete_{{$count}}">
    @method('DELETE')
    @csrf
</form>
<?php $count++; ?>
    ...
    @foreach ($infodetail as $info)
        ...
        <div class="col-sm-10">
            <select class="form-control round-form" name="tourist_id[]" >
                @foreach ($atts as $att)
                <option value="{{$att->tourist_id}}" {{($info->tourist_id == $att->tourist_id?'selected':'')}}>{{$att->tourist_name}}</option>
                @endforeach
              </select>
        </div>
        </div>
        ...
        <div class="form-group">
        <div class="col-sm-12">
            <button class="btn btn-theme04 btn-sm" type="button"onclick="document.getElementById('delete_{{$count}}').submit();"><i class="fa fa-trash-o "></i></button>
        </div>
        </div>
        </div>
        <?php $count++; ?>
